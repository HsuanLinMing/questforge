...
- `final_accuse`：指認（一定要有「我還不確定，交給大人」）
- `scene_10_ending_clear`：推理分足夠的結尾
- `scene_10_ending_nudge`：差一點（AI/霏霏補一句）
- `scene_10_ending_defer`：交給大人/先確認再結論的結尾
- `quit`：故事結束（固定）
...
- `final_accuse` 必須有 4 個選項（可依案件調整嫌疑人名單），其中一定包含：
  - 「我還不確定，交給大人」→ `next: "quit"`
- 其他三個為嫌疑人 → `next: "quit"`
- 不在 `final_accuse` 幫讀者複習線索、不提示正解（由引擎評分 + ending 分流呈現回饋）
...
- 推進時優先用自然對話、人物反應、說法矛盾、重新確認來帶出線索；不要寫成整理重點或推理教學
