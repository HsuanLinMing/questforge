STORY_NODES = {

# =================================================
# 🎉 開場：運動會前一天＋運動護照（世界觀一次講清楚）
# =================================================
"start": {
    "title": "運動會貼紙日",
    "narration":
        "今天是學校運動會的前一天。\n"
        "教室裡比平常熱鬧好多。\n\n"
        "牆上貼著大大的海報：跑步、跳遠、接力。\n"
        "窗外的操場也擺好了三角錐，風一吹，旗子啪啦啪啦響。\n\n"
        "每個人的桌上，都放著一本小小的本子。\n"
        "封面寫著：『運動護照』。\n\n"
        "老師拍了拍手說：\n"
        "『運動護照是記錄你們努力的。』\n"
        "『今天大家都可以貼「努力貼紙」，因為你們願意練習就很棒。』\n"
        "『明天比賽結束，得獎的人才會貼「第一名貼紙」。』\n\n"
        "老師笑著補一句：\n"
        "『貼好之後可以帶回家給爸爸媽媽看。』\n"
        "『我們也會在班級公布欄貼上大家的運動護照照片。』\n"
        "『讓每個人的努力都被看到。』\n\n"
        "桌上有一盒彩色貼紙。\n"
        "最上面那張「第一名貼紙」金金亮亮的，像小太陽。\n\n"
        "樂樂瞄了那張貼紙一眼，小聲說：\n"
        "『如果我明天拿到，我爸媽一定會超開心。』\n"
        "菲菲笑笑回他：\n"
        "『那我們今天先好好練習，明天再一起加油。』",
    "choices": [
        {"text": "去看看貼紙桌", "next": "scene_sticker_table"},
        {"text": "先去操場練習", "next": "scene_playground"},
        {"text": "先整理運動護照和書包", "next": "scene_bag"},
    ],
},

# =================================================
# 🧳 生活感支線：整理書包（帶出人物）
# =================================================
"scene_bag": {
    "title": "整理書包",
    "narration":
        "我把書包打開，鉛筆盒咔啦一聲彈開。\n"
        "菲菲摸摸口袋，皺了一下眉：\n"
        "『咦……我的橡皮擦又不見了。』\n\n"
        "樂樂忍不住笑：\n"
        "『姊姊真的很會把東西弄丟。』\n"
        "菲菲吐吐舌頭：\n"
        "『所以我更要用眼睛找。』\n\n"
        "這時我聽到後面有人說話。\n"
        "小狗東東抱著運動護照，抱得很緊。\n"
        "他沒跟人聊天，只一直看著貼紙桌的方向。\n\n"
        "旁邊的小熊阿力在翻鉛筆盒，翻得很大聲：\n"
        "『啊——我明天一定要贏回來！』\n\n"
        "小貓小芽把水壺放進袋子裡，動作快快的。\n"
        "她的書包上貼滿亮亮的貼紙，晃一下就閃一下。",
    "choices": [
        {"text": "去看看貼紙桌", "next": "scene_sticker_table"},
        {"text": "去操場練習", "next": "scene_playground"},
        {"text": "回座位看看大家在做什麼", "next": "scene_seat"},
    ],
},

# =================================================
# 🏷️ 貼紙桌前：細節線索（不明講）
# =================================================
"scene_sticker_table": {
    "title": "貼紙桌前",
    "narration":
        "貼紙被排得整整齊齊。\n"
        "努力貼紙有跑步的、有跳遠的、有接力棒。\n\n"
        "第一名貼紙金金的，被放在最上面。\n"
        "老師提醒大家：\n"
        "『第一名貼紙明天才用喔，今天先不要動它。』\n\n"
        "桌邊放著一小卷透明膠帶。\n"
        "膠帶的背紙有一小截翹起來，像被拉過一下。\n\n"
        "小貓小芽站得很近，又很快退後一步。\n"
        "她把手藏到身後，臉有點紅。\n\n"
        "小熊阿力擠到旁邊，嘴裡念念有詞：\n"
        "『明天我不要再輸了……我不要再輸了。』\n\n"
        "小狗東東沒有靠近。\n"
        "他站在比較遠的地方，眼睛一直盯著那張金色貼紙。",
    "choices": [
        {"text": "去操場練習", "next": "scene_playground"},
        {"text": "回座位看看大家", "next": "scene_seat"},
    ],
},

# =================================================
# 🪑 回座位：自然帶出「怪怪的」但不先下結論
# =================================================
"scene_seat": {
    "title": "回到座位",
    "narration":
        "我回到座位。\n"
        "有人在擦汗，有人在喝水，有人在翻運動護照。\n\n"
        "小狗東東把運動護照蓋得很緊，手指一直捏著封面角角。\n"
        "他沒有看別人。\n\n"
        "樂樂看了一眼，悄悄靠近我說：\n"
        "『他看起來好緊張喔。』\n"
        "菲菲也小聲說：\n"
        "『等一下如果真的有事，我們先找老師，不要自己吵起來。』",
    "choices": [
        {"text": "去操場練習", "next": "scene_playground"},
        {"text": "再去看看貼紙桌", "next": "scene_sticker_table"},
        {"text": "先去喝水休息", "next": "scene_water"},
    ],
},

"scene_water": {
    "title": "喝口水",
    "narration":
        "我們到飲水機旁邊喝水。\n"
        "走廊亮亮的，遠遠傳來操場的哨子聲。\n\n"
        "樂樂喝完水，小聲說：\n"
        "『我明天真的很想跑快一點。』\n"
        "菲菲點點頭：\n"
        "『想贏很正常啊。』\n"
        "『但如果心裡很急，我們就先把呼吸放慢。』",
    "choices": [
        {"text": "去操場練習", "next": "scene_playground"},
        {"text": "回教室準備貼努力貼紙", "next": "scene_return"},
    ],
},

# =================================================
# 🏃 操場練習：這裡不談貼紙不見（還沒發生）
# =================================================
"scene_playground": {
    "title": "操場練習",
    "narration":
        "操場上很熱鬧。\n"
        "有人練跑步，有人練接力交棒。\n\n"
        "樂樂跑完一圈，喘著氣：\n"
        "『我比昨天快一點點！』\n"
        "菲菲拍拍他的背：\n"
        "『很棒，今天的努力貼紙就是要貼這個。』\n\n"
        "不遠處，小熊阿力跑到一半停下來，跺了一下腳：\n"
        "『啊！我又踩線了！』\n\n"
        "小貓小芽一直整理鞋帶，整理得很快，像很怕來不及。\n\n"
        "小狗東東站在最後面。\n"
        "他沒有跑，只把運動護照抱在胸前，像抱著很重要的東西。\n"
        "他看著跑道，嘴巴動了一下，又沒說出來。",
    "choices": [
        {"text": "回教室準備貼努力貼紙", "next": "scene_return"},
        {"text": "去問東東：你還好嗎？", "next": "scene_check_dongdong"},
        {"text": "去看看阿力怎麼了", "next": "scene_check_ali"},
    ],
},

"scene_check_dongdong": {
    "title": "問問東東",
    "narration":
        "我走到東東旁邊。\n"
        "我沒有問他『你是不是做了什麼』。\n"
        "我只是說：\n"
        "『你還好嗎？你看起來有點緊。』\n\n"
        "東東嚇了一下，抱運動護照抱更緊。\n"
        "他小聲說：\n"
        "『我……我沒事。』\n\n"
        "他說完就低下頭。\n"
        "像是把話塞回心裡。",
    "choices": [
        {"text": "回教室", "next": "scene_return"},
        {"text": "去找菲菲和樂樂", "next": "scene_return"},
    ],
},

"scene_check_ali": {
    "title": "看看阿力",
    "narration":
        "阿力抓抓頭，臉紅紅的：\n"
        "『我明天不想再輸……我每次都差一點點。』\n\n"
        "樂樂聽到，嘴巴張了一下又閉起來。\n"
        "他像是想說什麼，但忍住了。\n\n"
        "菲菲先開口：\n"
        "『差一點點也很努力啊。』\n"
        "『明天我們一起加油。』",
    "choices": [
        {"text": "回教室", "next": "scene_return"},
    ],
},

# =================================================
# ‼️ 事件發生：回教室準備貼努力貼紙 → 發現第一名貼紙不見
# =================================================
"scene_return": {
    "title": "回到教室",
    "narration":
        "大家回到教室，準備把今天的努力貼紙貼進運動護照。\n\n"
        "老師把貼紙盒放到桌上：\n"
        "『來，今天每個人選一張努力貼紙。』\n"
        "『因為你們都練習了。』\n\n"
        "老師又指著盒子最上面那張金色貼紙：\n"
        "『第一名貼紙明天才用，我們先放好。』\n\n"
        "可是下一秒，老師的聲音停了一下：\n"
        "『咦？第一名貼紙呢？』\n\n"
        "教室安靜了一點點。\n"
        "像有人把大家的聲音先收起來。",
    "choices": [
        {"text": "先看看貼紙桌附近", "next": "scene_problem"},
        {"text": "先去找老師（小聲說）", "next": "scene_teacher"},
        {"text": "先安慰一下大家，別慌", "next": "scene_calm_room"},
    ],
},

"scene_calm_room": {
    "title": "先讓教室不要亂",
    "narration":
        "樂樂看著貼紙盒，眉頭皺起來。\n"
        "他吸一口氣，小聲說：\n"
        "『我好想大叫……可是我知道這樣會更亂。』\n\n"
        "菲菲點點頭：\n"
        "『我們先把聲音放小。』\n"
        "『讓老師可以好好處理。』",
    "choices": [
        {"text": "去貼紙桌附近看看", "next": "scene_problem"},
        {"text": "去找老師", "next": "scene_teacher"},
    ],
},

"scene_problem": {
    "title": "貼紙盒旁邊",
    "narration":
        "貼紙盒還在桌上。\n"
        "努力貼紙都在。\n"
        "只有那張金色第一名貼紙不見了。\n\n"
        "桌邊那卷透明膠帶的背紙，比剛剛更翹。\n"
        "地上還有一小截透明背紙，黏著一點點灰。\n\n"
        "小貓小芽把書包拉鍊拉得很緊。\n"
        "她的手指頭一直在摸書包上的亮亮貼紙。\n\n"
        "小熊阿力一直看著貼紙盒，嘴巴抿得緊緊的。\n\n"
        "小狗東東把運動護照蓋住，整本抱在胸前。\n"
        "他站起來又坐下來，像坐不住。",
    "choices": [
        {"text": "小聲問阿力：你剛剛有看到什麼嗎？", "next": "ask_ali"},
        {"text": "小聲問小芽：你剛剛有靠近貼紙盒嗎？", "next": "ask_mei"},
        {"text": "小聲問東東：你怎麼了？", "next": "ask_dongdong"},
        {"text": "先去找老師（把看到的說清楚）", "next": "scene_teacher"},
    ],
},

"ask_ali": {
    "title": "問阿力",
    "narration":
        "我小聲問：\n"
        "『阿力，你剛剛有看到誰靠近貼紙盒嗎？』\n\n"
        "阿力先搖頭，後來又小小聲說：\n"
        "『我剛剛看到……東東站在桌邊很久。』\n"
        "『他一直看著盒子。』\n\n"
        "阿力說完趕快補一句：\n"
        "『我不確定啦，我只是看到他站很久。』",
    "choices": [
        {"text": "去問東東", "next": "ask_dongdong"},
        {"text": "去問小芽", "next": "ask_mei"},
        {"text": "回去再看貼紙桌", "next": "scene_problem"},
        {"text": "進入指認前先停一下", "next": "final_accuse"},
    ],
},

"ask_mei": {
    "title": "問小芽",
    "narration":
        "我小聲問：\n"
        "『小芽，你剛剛有靠近貼紙盒嗎？』\n\n"
        "小芽臉一下子更紅：\n"
        "『我……我有去拿膠帶。』\n"
        "『因為我的運動護照封面翹起來。』\n\n"
        "她把書包拉鍊拉得更緊：\n"
        "『可是我沒有拿金色貼紙。真的沒有。』\n\n"
        "她說的很快，像在趕時間。",
    "choices": [
        {"text": "回去再看貼紙桌", "next": "scene_problem"},
        {"text": "去問東東", "next": "ask_dongdong"},
        {"text": "去找老師", "next": "scene_teacher"},
        {"text": "進入指認前先停一下", "next": "final_accuse"},
    ],
},

"ask_dongdong": {
    "title": "問東東",
    "narration":
        "我走到東東旁邊。\n"
        "我沒有說『是不是你拿的』。\n"
        "我只是問：\n"
        "『東東，你怎麼一直抱著運動護照？你看起來好緊。』\n\n"
        "東東的耳朵（或頭）垂了一點點。\n"
        "他嘴巴抖一下，小聲說：\n"
        "『我……我想明天大家可以看我一眼。』\n\n"
        "他又趕快搖頭：\n"
        "『我不是故意要讓老師生氣……』",
    "choices": [
        {"text": "先不問名字，去找菲菲和樂樂一起整理", "next": "final_accuse"},
        {"text": "直接去跟老師說（可能會指錯）", "next": "ending_wrong"},
        {"text": "去找老師（只說看到的，不說猜的）", "next": "ending_check"},
    ],
},

# =================================================
# 👩‍🏫 老師角色：早出場＋安全感
# =================================================
"scene_teacher": {
    "title": "找老師",
    "narration":
        "老師走過來，沒有問：『是誰拿的？』\n"
        "她先說：\n"
        "『謝謝你們沒有吵。』\n"
        "『我們慢慢來。』\n\n"
        "老師看著大家：\n"
        "『你們可以跟我說：你看到什麼、你聽到什麼。』\n"
        "『不要用猜的，也不要指著人。』\n\n"
        "老師把貼紙盒先收好，放到講桌上。\n"
        "『先讓它安全。』她說。",
    "choices": [
        {"text": "回去再觀察一次", "next": "scene_problem"},
        {"text": "進入指認前先停一下", "next": "final_accuse"},
        {"text": "把看到的整理好再說", "next": "ending_check"},
    ],
},

# =================================================
# 🧠 指認前必出：情緒降溫對話（核心）
# =================================================
"final_accuse": {
    "title": "停一下，先把心放慢",
    "narration":
        "樂樂的眉毛皺得緊緊的。\n"
        "他小聲說：\n"
        "『我心裡很不舒服。』\n"
        "『那張第一名貼紙明天才用，現在不見了，我覺得怪怪的。』\n\n"
        "他握了握拳，又慢慢放開：\n"
        "『我好想直接說名字……但我不想亂發脾氣。』\n\n"
        "菲菲看著他，點點頭：\n"
        "『你能忍住，已經很厲害了。』\n"
        "『我們先做一件事：只講我們看到的。』\n\n"
        "菲菲輕聲說：\n"
        "『如果不確定，就交給老師。』\n"
        "『我們可以說：我還在找。』\n\n"
        "樂樂吸一口氣：\n"
        "『好。先講看到的，不講猜的。』",
    "choices": [
        {"text": "把看到的整理好，跟老師說", "next": "ending_check"},
        {"text": "我覺得就是某個人！（可能會指錯）", "next": "ending_wrong"},
        {"text": "回去再看一次貼紙桌", "next": "scene_problem"},
    ],
},

# =================================================
# ✅ A：正確做法—先說看到的 → 老師接手
# =================================================
"ending_check": {
    "title": "把話說清楚",
    "narration":
        "我們走到老師旁邊。\n"
        "我慢慢說：\n"
        "『我看到貼紙盒旁邊有透明背紙掉在地上。』\n"
        "『我也看到有人一直抱著運動護照，坐不住。』\n"
        "『還有人去拿過膠帶。』\n\n"
        "我補一句：\n"
        "『這些是我看到的，我不確定是誰。』\n\n"
        "老師點點頭：\n"
        "『謝謝你講得很清楚。』\n"
        "『你沒有指人，你做得很好。』",
    "choices": [
        {"text": "讓老師問清楚（安全結案）", "next": "ending_result"},
    ],
},

"ending_result": {
    "title": "事情被安全接住了",
    "narration":
        "老師請大家先坐好。\n"
        "她的聲音不大，但很穩：\n"
        "『我們先把事情說清楚。』\n\n"
        "老師看著東東：\n"
        "『東東，你剛剛一直抱著運動護照。你想說什麼嗎？』\n\n"
        "東東的眼睛紅紅的。\n"
        "他小聲說：\n"
        "『我想明天被大家看到。』\n"
        "『我怕我又跑很慢……我就……太急了。』\n\n"
        "他低頭把一張金色貼紙拿出來。\n"
        "『我剛剛把它撕下來了。』\n\n"
        "老師輕輕接過貼紙，放回盒子最上面：\n"
        "『謝謝你說出來。』\n"
        "『想被看到是正常的。』\n"
        "『但不可以先把明天的貼紙拿走。』\n\n"
        "老師轉向全班：\n"
        "『今天我們貼努力貼紙。』\n"
        "『明天的第一名貼紙，要靠比賽後的結果。』\n\n"
        "老師也說：\n"
        "『我會跟家長說明今天的事，讓東東有人陪著練習。』\n"
        "『也讓他學會：心裡很急的時候，可以先找老師說。』\n\n"
        "樂樂小聲對我說：\n"
        "『原來找老師不是告狀，是讓事情變安全。』\n\n"
        "菲菲笑了一下：\n"
        "『對。說清楚，比吵起來更勇敢。』",
    "lesson": [
        "菲菲說：『先講看到的，不講猜的。』",
        "樂樂說：『心裡不舒服，我可以先停一下。』",
        "老師說：『不確定就找大人，事情才安全。』",
        "你們學到：想被看到可以用努力，不用急著做小動作。"
    ],
    "choices": [
        {"text": "進入尾聲", "next": "epilogue"},
        {"text": "再玩一次", "next": "start"},
        {"text": "離開遊戲", "next": "quit"},
    ],
},

# =================================================
# ❌ B：錯誤指認分支（教孩子不確定不要指人）
# =================================================
"ending_wrong": {
    "title": "先停一下，不要急著指人",
    "narration":
        "如果我們急著說出某個名字——\n"
        "被說到的人會嚇一跳，臉一下子變白。\n\n"
        "老師立刻走過來，語氣很溫柔：\n"
        "『謝謝你願意說。』\n"
        "『但我想先問：這是你真的看到的嗎？』\n\n"
        "樂樂的臉紅紅的。\n"
        "他小聲說：\n"
        "『我……我只是很不舒服，就想快點說出來。』\n\n"
        "菲菲輕輕說：\n"
        "『不舒服是真的。』\n"
        "『但不確定的時候，我們可以說：我還在找。』\n\n"
        "老師點點頭：\n"
        "『對。你們回去再看一次。』\n"
        "『我會一直在，也會一起處理。』",
    "lesson": [
        "你們學到：不確定時不要指人。",
        "你可以說：『我還在找，我想請老師一起看。』",
        "找大人不是告狀，是讓大家更安全。"
    ],
    "choices": [
        {"text": "回到貼紙盒附近再觀察", "next": "scene_problem"},
        {"text": "回去找老師，把看到的整理好", "next": "ending_check"},
        {"text": "重新開始", "next": "start"},
    ],
},

# =================================================
# 🌙 尾聲
# =================================================
"epilogue": {
    "title": "尾聲：明天一起加油",
    "narration":
        "下課鐘聲響起。\n"
        "教室又慢慢有了聲音。\n\n"
        "東東坐在位子上，手指還在抖。\n"
        "老師沒有讓他一個人扛著。\n"
        "老師坐到他旁邊，小聲跟他說話。\n\n"
        "過一會兒，東東抬起頭，小小聲說：\n"
        "『我明天會好好比。』\n\n"
        "樂樂看著跑道海報，握拳又放開：\n"
        "『我也要。』\n\n"
        "菲菲笑著說：\n"
        "『明天我們用努力去拿貼紙。』\n"
        "『回家也可以大聲跟爸爸媽媽說：我有認真練習。』",
    "choices": [
        {"text": "再玩一次", "next": "start"},
        {"text": "離開遊戲", "next": "quit"},
    ],
},

"quit": {
    "title": "離開",
    "narration":
        "菲菲和樂樂揮揮手：\n"
        "『明天運動會，加油！』",
    "choices": [],
},

}
